...
async function main() {
    ...
    try {
        ...
        let fabricProxy = new FabricProxy();
        ...
        for (const contract of metadata['contracts']){
            ...
            for (const transaction of contract['operations']) {
                ...
                if (commit) {
                    ...
                    app.post(`/${contract['namespace']}/${name}`, async (req: express.Request, res: express.Response) => {
        ...
        writeFileSync('swagger.json',JSON.stringify(swagger));
        app.listen(port);

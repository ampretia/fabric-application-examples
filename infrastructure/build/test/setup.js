...
let root = process.env.DEVROOT;
if (!root){
	console.error('Need root set')
	process.exit(1);
}

let company = process.env.COMPANY;
let bnTemplatePath = path.join(__dirname, 'network-template');
let bnTestDir = path.join(root, company,'basic-network');

// by default for running the tests print debug to a file
let debugPath = path.join(bnTestDir, 'logs/test-debug.log');
console.log('\n####################################################');
...
gulp.task('clean-up', function() {
	...
	// fs.removeSync(bnTestDir);
	// fs.ensureFileSync(bnTestDir);
	// return fs.ensureFileSync(debugPath);
});

console.log('\n####################################################');
console.log('BasicNetworkSamplePath: %s', bnTemplatePath);
console.log('BasicNetworkTestDir: %s', bnTestDir);
console.log('####################################################\n');
...
gulp.task('docker-copy', ['clean-up'], function() {
	...
	gulp.src([
		...
		path.join(root, 'contracts') // copy the empty folder only
	], {base: bnTemplatePath})
		.pipe(gulp.dest(bnTestDir));
...
gulp.task('docker-clean', ['docker-copy'], () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
			...
			util.format('docker-compose -f %s down', fs.realpathSync(path.join(bnTestDir, 'docker-compose.yml')))
		], {
...
gulp.task('docker-cli-ready', ['docker-clean'], () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
			...
			util.format('docker-compose -f %s up -d cli', fs.realpathSync(path.join(bnTestDir, 'docker-compose.yml')))
		]));
});
...
gulp.task('docker-ready', ['generate-config'], () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
			// make sure that necessary containers are up by docker-compose
			util.format('docker-compose -f %s up -d', fs.realpathSync(path.join(bnTestDir, 'docker-compose.yml')))
		]));
});

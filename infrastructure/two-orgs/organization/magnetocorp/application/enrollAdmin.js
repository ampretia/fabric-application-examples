...
const ccpPath = path.resolve(__dirname, '..', '..', 'networkConnection.yaml');
...
async function main() {
    try {
        ...
        const adminExists = await wallet.exists('Admin@magnetocorp.example.com');
        if (adminExists) {
        ...
        wallet.import('Admin@magnetocorp.example.com', identity);
